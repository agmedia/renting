                ...
                <div class="accordion mt-n1" id="shop-categories">
                    @foreach ($categories as $category)
                        <div class="accordion-item border-bottom">
                            <h3 class="accordion-header"><a href="javascript:void(0)" class="accordion-button py-2 none collapsed" wire:click="changeSelected('{{ $category->slug }}')" role="link">{{ $category->title }} <span class="badge bg-secondary ms-2 position-absolute end-0">{{ $category->products()->count() }}</span></a></h3>
                        </div>
                    @endforeach
                </div>
            </div>
